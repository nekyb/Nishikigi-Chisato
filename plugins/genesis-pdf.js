...
> _*By Soblend | Development Studio Creative*_`
            ...
            const organizedImages = await this.analyzeAndOrganizeImages(session.images)
...
> _*By Soblend | Development Studio Creative*_`,
                contextInfo: {
                    externalAdReply: {
                        ...
                        thumbnailUrl: "https://i.ibb.co/7XqG5Zp/pdf-icon.jpg",
...
> _*By Soblend | Development Studio Creative*_`
    ...
    async analyzeAndOrganizeImages(images) {
        try {
            const genAI = new GoogleGenerativeAI('AIzaSyDR01zJZyCyWUgF7QbVFZXZTGlLt1bMf9M')
...
Responde en formato: TIPO | TEXTO_PRINCIPAL | SECUENCIA | PAGINA`
    ...
    extractOrder(analysis, defaultOrder) {
        const pageMatch = analysis.match(/p[áa]gina\s*(\d+)|page\s*(\d+)|(\d+)\s*\/\s*\d+/i)
        if (pageMatch) {
            return parseInt(pageMatch[1] || pageMatch[2] || pageMatch[3])
        }
        
        const stepMatch = analysis.match(/paso\s*(\d+)|step\s*(\d+)|secuencia\s*(\d+)/i)
        if (stepMatch) {
